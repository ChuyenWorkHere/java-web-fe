...
@WebServlet("/mega-admin/manage-account")
public class MegaAdminView extends HttpServlet {
    private static final long serialVersionUID = 1L;


    public MegaAdminView() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.setAttribute("view", "mega-admin");
        ...
        List<User> users = userDAO.findAllPage(currentPage, 12, keyword, status, dir, orderBy, -1);
        ...
        for(User user : users) {
            ...
            if(user.getRole().getRoleId() == 1) {
                out.append("                        <button class='btn btn-warning' ");
                out.append("                            data-bs-toggle='modal' data-bs-target='#smallModalUpdate-"+user.getUserId()+"'> ");
                out.append("                            <i class='bi bi-arrow-down-circle text-white'></i></button>");
            }else {
                out.append("                        <button class='btn btn-warning' ");
                out.append("                            data-bs-toggle='modal' data-bs-target='#smallModalUpdate-"+user.getUserId()+"'> ");
                out.append("                            <i class='bi bi-arrow-up-circle text-white'></i></button>");
            }

            out.append("    <!-- Start Small Modal Update-->");
            out.append("    <div class=\"modal fade\" id=\"smallModalUpdate-"+user.getUserId()+"\" tabindex=\"-1\">");
            out.append("      <div class=\"modal-dialog modal-sm\">");
            out.append("        <div class=\"modal-content\">");
            out.append("        <form action=\""+request.getContextPath() + "/mega-admin/account/update" +"\" method=\"post\">");
            out.append("          <div class=\"modal-header bg-warning text-white\">");
            out.append("            <h5 class=\"modal-title \">"+user.getFullname()+"</h5>");
            out.append("            <button type=\"button\" class=\"btn-close btn-close-white\" data-bs-dismiss=\"modal\" aria-label=\"Close\"></button>");
            out.append("          </div>");
            out.append("          <div class=\"modal-body\">");
            if(user.getRole().getRoleId() == 1) {
                out.append("            Bạn có muốn chuyển tài khoản "+user.getFullname()+" sang vai trò người dùng không?");
            }else {
                out.append("            Bạn có muốn chuyển tài khoản "+user.getFullname()+" sang vai trò quản lý không?");
            }
            out.append("            <input type=\"hidden\" name=\"userId\" value=\""+user.getUserId()+"\">");
            if(user.getRole().getRoleId() == 1) {
                out.append("            <input type=\"hidden\" name=\"roleId\" value=\"2\">");
            }else {
                out.append("            <input type=\"hidden\" name=\"roleId\" value=\"1\">");
            }
            out.append("          </div>");
            out.append("          <div class=\"modal-footer\">");
            out.append("            <button type=\"button\" class=\"btn btn-secondary\" data-bs-dismiss=\"modal\">Huỷ</button>");
            out.append("            <button type=\"submit\" class=\"btn btn-warning text-white\">");
            out.append("              Xác nhận");
            out.append("            </button>");
            out.append("          </div>");
            out.append("        </form>");
            out.append("        </div>");
            out.append("      </div>");
            out.append("    </div><!-- End Small Modal Update-->");
        ...
        out.append("              </div>");
        out.append("              <form class=\"container py-4 d-flex flex-column align-items-start gap-3\" action=\"/Furniture/mega-admin/manage-account\" method=\"GET\">");
...
// Next button
    ...
    private String getPageUrl(int page, String keyword, String status, String dir, String orderBy){
        String url = "../mega-admin/manage-account?pageNo=" + page;
